...
model_f = model_MAF.PC_MAF(dim_condition=num_inputs+len(positions),
...
optimizer = torch.optim.Adam(model_f.model.parameters(), lr=learning_rate, betas=(0.8, 0.9),
                             eps=1e-6, weight_decay=2e-5)
if use_hvd:
    optimizer = hvd.DistributedOptimizer(optimizer, named_parameters=model_f.named_parameters())
